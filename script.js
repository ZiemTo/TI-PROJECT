class List {
  ...
  static ResizeTextarea(task) {
    ...
    task.addEventListener("input", List.OnInput, false);
  }
  static OnInput() {
  ...
  static Cross(checkbox) {
    if (checkbox.checked) {
      ...
      List.ResizeTextarea(checkbox.nextSibling.nextSibling.firstChild);
    } else {
...
function handleMutation(mutationsList, observer) {
  mutationsList.forEach((mutation) => {
    ...
    savetasks.forEach((savetask) => {
      if (
        !savetask.value.length < 1 &&
        !savetask.parentElement.previousSibling.previousSibling.checked
      ) {
...
function SetPrevious() {
  if (localStorage.getItem("background") != null) {
    document
      ...
      .setAttribute("src", localStorage.getItem("background"));
  }
  if (localStorage.getItem("savedtasks") != null) {
    ...
    savedtasks.forEach((task) => {
      ...
      newspan.setAttribute("class", "spantask");
      newspan.innerHTML =
        '<input type="checkbox" name="task" onclick="Cross(this)" class="taskcheck "> <label for="task"><textarea class="task"  maxlength="80" oninput="ResizeTextarea(this)" placeholder="Write your task here" ></textarea></label>';
      document.querySelector("#tasks").appendChild(newspan);
  ...
  if (getCookie("minutes").length > 0) {
    document
      .querySelector("#time")
      .firstChild.nextSibling.setAttribute("value", getCookie("minutes"));
  } else
    document
      .querySelector("#time")
      .firstChild.nextSibling.setAttribute("value", "00");
  if (getCookie("seconds").length > 0) {
    document
      .querySelector("#time")
      .lastChild.previousSibling.setAttribute("value", getCookie("seconds"));
  } else
    document
      .querySelector("#time")
      .lastChild.previousSibling.setAttribute("value", "00");
}
...
function SetDefaultValues() {
  ...
  cookieMinutesValue =
    document.querySelector("#defaulttimer").firstChild.nextSibling.value;
  cookieSecondsValue =
    document.querySelector("#defaulttimer").lastChild.previousSibling.value;
  ...
  document.cookie =
    cookieMinutes + "=" + cookieMinutesValue + ";expires=" + exp;
  document.cookie =
    cookieSeconds + "=" + cookieSecondsValue + ";expires=" + exp;
}
function Submit() {
